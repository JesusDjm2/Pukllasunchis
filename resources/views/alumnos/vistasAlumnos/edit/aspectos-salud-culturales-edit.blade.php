<div class="row">
    ...
    <div class="col-lg-6 mb-2 mt-2">
        <label for="problemas_salud">¿Presenta algún problema de salud? Sí su respuesta es Sí, continue con la pregunta
            2. Caso contrario, continúe con la pregunta 4.:</label><br>
    ...
    <div class="col-lg-6 mb-2">
        ...
        <select class="form-control form-control-sm @error('ultima_consulta') is-invalid @enderror"
            ...
            <option value="1" {{ old('ultima_consulta', $alumno->ultima_consulta) == '1' ? 'selected' : '' }}>Falta
                de recursos económicos</option>
            <option value="0" {{ old('ultima_consulta', $alumno->ultima_consulta) == '0' ? 'selected' : '' }}>Falta
                de tiempo</option>
        </select>
        @error('ultima_consulta')
            <div class="invalid-feedback">{{ $message }}</div>
    ...
    <div class="col-lg-6 mb-2">
        ...
        <select class="form-control form-control-sm @error('tipo_seguro') is-invalid @enderror" name="tipo_seguro">
            ...
            <option value="EsSalud" {{ old('tipo_seguro', $alumno->tipo_seguro) == 'EsSalud' ? 'selected' : '' }}>
                EsSalud</option>
            <option value="Seguro Ejército"
                {{ old('tipo_seguro', $alumno->tipo_seguro) == 'Seguro Ejército' ? 'selected' : '' }}>Seguro Ejército
            </option>
            <option value="Seguro Integral de Salud (SIS)"
                {{ old('tipo_seguro', $alumno->tipo_seguro) == 'Seguro Integral de Salud (SIS)' ? 'selected' : '' }}>
                Seguro Integral de Salud (SIS)</option>
            <option value="Seguro de la Marina de Guerra del Perú"
                {{ old('tipo_seguro', $alumno->tipo_seguro) == 'Seguro de la Marina de Guerra del Perú' ? 'selected' : '' }}>
                Seguro de la Marina de Guerra del Perú</option>
            <option value="Seguro PNP"
                {{ old('tipo_seguro', $alumno->tipo_seguro) == 'Seguro PNP' ? 'selected' : '' }}>Seguro PNP</option>
            <option value="Seguro FAP"
                {{ old('tipo_seguro', $alumno->tipo_seguro) == 'Seguro FAP' ? 'selected' : '' }}>Seguro FAP</option>
        </select>
        @error('tipo_seguro')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    ...
    <div class="col-lg-6 mb-2 mt-2">
        ...
        <select class="form-control form-control-sm @error('frecuencia_lectura') is-invalid @enderror"
            ...
            <option value="Todos los días"
                {{ old('frecuencia_lectura', $alumno->frecuencia_lectura) == 'Todos los días' ? 'selected' : '' }}>
                Todos los días</option>
            <option value="1 vez por semana"
                {{ old('frecuencia_lectura', $alumno->frecuencia_lectura) == '1 vez por semana' ? 'selected' : '' }}>1
                vez a la semana</option>
            <option value="Cada 15 días"
                {{ old('frecuencia_lectura', $alumno->frecuencia_lectura) == 'Cada 15 días' ? 'selected' : '' }}>Cada
                15 días</option>
            <option value="1 vez al mes"
                {{ old('frecuencia_lectura', $alumno->frecuencia_lectura) == '1 vez al mes' ? 'selected' : '' }}>1 vez
                al mes</option>
            <option value="Más de 1 mes"
                {{ old('frecuencia_lectura', $alumno->frecuencia_lectura) == 'Más de 1 mes' ? 'selected' : '' }}>Más de
                1 mes</option>
        </select>
    ...
    <div class="col-lg-6 mb-2 mt-2">
        ...
        <select class="form-control form-control-sm @error('acceso_lectura') is-invalid @enderror"
            name="acceso_lectura">
            <option value="" selected disabled>Seleccionar</option>
            <option value="Las compra"
                {{ old('acceso_lectura', $alumno->acceso_lectura) == 'Las compra' ? 'selected' : '' }}>Las compra
            </option>
            <option value="Trabajo u oficina"
                {{ old('acceso_lectura', $alumno->acceso_lectura) == 'Trabajo u oficina' ? 'selected' : '' }}>Trabajo u
                oficina</option>
            <option value="Las lee por internet"
                {{ old('acceso_lectura', $alumno->acceso_lectura) == 'Las lee por internet' ? 'selected' : '' }}>Las
                lee por internet</option>
        </select>
    ...
    <div class="col-lg-6 mb-2 mt-2">
        ...
        <select class="form-control form-control-sm @error('visitas_museos') is-invalid @enderror"
            name="visitas_museos">
            <option value="" selected disabled>Seleccionar</option>
            <option value="El último mes"
                {{ old('visitas_museos', $alumno->visitas_museos) == 'El último mes' ? 'selected' : '' }}>El último mes
            </option>
            <option value="En los últimos 3 meses"
                {{ old('visitas_museos', $alumno->visitas_museos) == 'En los últimos 3 meses' ? 'selected' : '' }}>En
                los últimos 3 meses</option>
            <option value="En los últimos 6 meses"
                {{ old('visitas_museos', $alumno->visitas_museos) == 'En los últimos 6 meses' ? 'selected' : '' }}>En
                los últimos 6 meses</option>
            <option value="En los últimos 12 meses"
                {{ old('visitas_museos', $alumno->visitas_museos) == 'En los últimos 12 meses' ? 'selected' : '' }}>En
                los últimos 12 meses</option>
            <option value="Más de 12 meses"
                {{ old('visitas_museos', $alumno->visitas_museos) == 'Más de 12 meses' ? 'selected' : '' }}>Más de 12
                meses</option>
        </select>
    ...
    <div class="col-lg-6 mb-2 mt-2">
        ...
        <select class="form-control form-control-sm @error('actividades_internet') is-invalid @enderror"
            ...
            <option value="Enviar/Recibir correos"
                {{ old('actividades_internet', $alumno->actividades_internet) == 'Enviar/Recibir correos' ? 'selected' : '' }}>
                Enviar/Recibir correos</option>
            <option value="Buscar información para aprender algo nuevo"
                {{ old('actividades_internet', $alumno->actividades_internet) == 'Buscar información para aprender algo nuevo' ? 'selected' : '' }}>
                Buscar información para aprender algo nuevo</option>
            <option value="Entretenimiento/Música"
                {{ old('actividades_internet', $alumno->actividades_internet) == 'Entretenimiento/Música' ? 'selected' : '' }}>
                Entretenimiento/Música</option>
            <option value="Informarse sobre actualidad de la prensa/medios"
                {{ old('actividades_internet', $alumno->actividades_internet) == 'Informarse sobre actualidad de la prensa/medios' ? 'selected' : '' }}>
                Informarse sobre actualidad de la prensa/medios</option>
            <option value="Otros"
                {{ old('actividades_internet', $alumno->actividades_internet) == 'Otros' ? 'selected' : '' }}>Otros
            </option>
        </select>
    ...
    <div class="col-lg-6 mb-2 mt-2">
        ...
        @foreach ($opcionesHabilidades as $talentos)
            <label class="form-check">
                <input class="form-check-input" type="checkbox" name="habilidades[]" value="{{ $talentos }}"
                    {{ in_array($talentos, $alumno->habilidades) ? 'checked' : '' }}>
                {{ $talentos}}
            </label>
        @endforeach
        @error('habilidades')
    ...
    <div class="col-lg-6 mb-2 mt-2">
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="tiempo_libre" id="tiempo_libre_si" value="1"
                {{ old('tiempo_libre', $alumno->tiempo_libre) == '1' ? 'checked' : '' }}>
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="tiempo_libre" id="tiempo_libre_no" value="0"
                {{ old('tiempo_libre', $alumno->tiempo_libre) == '0' ? 'checked' : '' }}>
