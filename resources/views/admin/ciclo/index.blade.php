                    ...
                    <div class="card-header bg-secondary" id="heading{{ $programaId }}">
                        <h2 class="mb-0">
                            <button class="btn btn-link text-white font-weight-bold" type="button" data-toggle="collapse"
                                data-target="#collapse{{ $programaId }}" aria-expanded="true"
                                aria-controls="collapse{{ $programaId }}">
                                {{ $programaNombre }}
                            </button>
                        </h2>
                    </div>
                    <div id="collapse{{ $programaId }}" class="collapse" aria-labelledby="heading{{ $programaId }}"
                        data-parent="#accordionCiclos">
                        <div class="card-body">
                            <div class="row">
                                @php
                                    $totalAlumnos = 0;
                                        ...
                                        <div class="col-lg-2">
                                            <div class="card mt-2 mb-2 p-2 text-center">
                                                <h5 class="mb-4" style="font-family: emoji">
                                                    <a
                                                        href="{{ route('ciclo.show', ['ciclo' => $ciclo->id]) }}">{{ $ciclo->nombre }}</a>
                                                </h5>
                                                <p>
                                                    @if (in_array($programaId, [3, 4, 5]))
                                                        {{ $ciclo->alumnos_b_validos_count }} Alumnos
                                                        @php $totalAlumnos += $ciclo->alumnos_b_validos_count; @endphp
                                                    @else
                                                        {{ $ciclo->alumnos_validos_count }} Alumnos
                                                        @php $totalAlumnos += $ciclo->alumnos_validos_count; @endphp
                                                    @endif
                                                </p>
                                                <div class="row">
                                                    <div class="col-4">
