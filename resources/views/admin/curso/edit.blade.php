...
@section('contenido')
    ...
    <div class="cf-page">
        <div class="cf-hero" id="cfHero">
            <div class="d-flex align-items-center" style="gap: .9rem;">
                <div class="cf-hero-icon"><i class="fas fa-pen-to-square"></i></div>
                <div>
        ...
        <form action="{{ route('curso.update', $curso->id) }}" method="POST" autocomplete="off" id="cfForm">
            @csrf
            @method('PUT')
            <div class="cf-layout">
              <div class="cf-col-left">
                        ...
                        <div class="cf-field">
                            ...
                            <select id="programa_id" name="programa_id" class="cf-control" required>
                                ...
                                @foreach ($programas as $prog)
                                    <option value="{{ $prog->id }}" {{ $programa->id == $prog->id ? 'selected' : '' }}>
                                        {{ $prog->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="cf-field">
                            ...
                            <select id="ciclo_id" name="ciclo_id" class="cf-control" required>
                                @foreach ($ciclos as $ciclo)
                                    <option value="{{ $ciclo->id }}" {{ $curso->ciclo_id == $ciclo->id ? 'selected' : '' }}>
                                        {{ $ciclo->nombre }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="cf-card">
                    ...
                    <div class="cf-field">
                        ...
                        <textarea name="sumilla" id="sumilla" maxlength="1000"
                            ...
                            rows="4" placeholder="Describe brevemente el propósito y alcance del curso…">{{ old('sumilla', $curso->sumilla ?? '') }}</textarea>
                ...
                <div class="cf-card cf-card--competencias">
                    ...
                    @php $competenciasSeleccionadas = $curso->competencias->pluck('id')->toArray(); @endphp
                    <div class="cf-comp-grid" id="cfCompGrid">
                        @forelse ($competencias as $competencia)
                            <label class="cf-comp-card {{ in_array($competencia->id, $competenciasSeleccionadas) ? 'is-checked' : '' }}"
                                data-comp-nombre="{{ strtolower($competencia->nombre) }}">
                                <input type="checkbox" name="competencias[]" value="{{ $competencia->id }}"
                                    {{ in_array($competencia->id, $competenciasSeleccionadas) ? 'checked' : '' }}>
    ...
    <script>
        (function() {
            ...
            // ── Ciclos dinámicos según el Programa elegido (mantiene el ciclo actual si aplica) ──
            var selectPrograma = document.getElementById('programa_id');
            var selectCiclo = document.getElementById('ciclo_id');
            var cicloActualId = '{{ $curso->ciclo_id }}';
            selectPrograma.addEventListener('change', function() {
                ...
                fetch('/obtener-ciclos/' + programaId)
                    ...
                    .then(function(data) {
                        ...
                        selectCiclo.value = cicloActualId;
                    });
            });
            ...
            window.addEventListener('pageshow', function(event) {
                ...
                submitBtn.innerHTML = '<i class="fas fa-save"></i>Actualizar curso';
            });
