...
@push('styles')
    <style>
        ...
        input:checked+.cea-slider {
            background: #1cc88a;
        }

        input:checked+.cea-slider::before {
...
<div class="cea-field-group">
    ...
    <div class="form-group">
        ...
        <input type="text" name="nombre" class="form-control form-control-lg @error('nombre') is-invalid @enderror"
            value="{{ old('nombre', $curso->nombre ?? '') }}" placeholder="Ej: Idioma Quechua — Qhichwa Simi" required>
        @error('nombre')
    ...
    <div class="form-group mb-0">
        ...
        <textarea name="descripcion" class="form-control @error('descripcion') is-invalid @enderror" rows="3"
            placeholder="Descripción breve que verá el alumno antes de inscribirse...">{{ old('descripcion', $curso->descripcion ?? '') }}</textarea>
        @error('descripcion')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
<div class="cea-field-group">
    ...
    @if (!empty($curso->imagen))
        ...
        <p class="small text-muted mb-2"><i class="fas fa-check-circle text-success mr-1"></i>Imagen actual — sube una
...
<div class="cea-field-group">
    ...
    <div class="form-group mb-0">
        <label class="font-weight-bold small">Asignar docente <span
        ...
        @error('docente_id')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        <small class="form-text text-muted">El docente podrá ver y editar el contenido de este curso desde su
            panel.</small>
    </div>
</div>
