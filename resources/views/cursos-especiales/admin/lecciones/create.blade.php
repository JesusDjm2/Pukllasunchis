...
@section('titulo', 'Nueva Lección — ' . $unidad->nombre)
...
.cea-breadcrumb a:hover { text-decoration: underline; }
.cea-form-footer { display: flex; gap: .75rem; flex-wrap: wrap; margin-top: 1rem; }
.cea-submit-btn {
...
<div class="container-fluid cea-form-page">
    <div class="cea-form-header">
        <div class="cea-breadcrumb mb-2">
            ...
            <i class="fas fa-chevron-right fa-xs"></i>
            <span>{{ Str::limit($unidad->nombre, 30) }}</span>
        </div>
        <h1><i class="fas fa-book-open mr-2" style="color:#4e73df"></i>Nueva Lección</h1>
    </div>
    ...
    <form action="{{ route($rp . '.lecciones.store', [$curso, $nivel, $unidad]) }}" method="POST">
        @csrf
        @include('cursos-especiales.admin.lecciones._form')
        <div class="cea-form-footer">
            <button type="submit" class="cea-submit-btn">
                <i class="fas fa-save"></i> Guardar Lección
            </button>
            <a href="{{ route($rp . '.show', $curso) }}" class="cea-cancel-btn">
