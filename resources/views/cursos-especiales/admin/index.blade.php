...
@push('styles')
<style>
.ce-admin-page { padding-bottom: 2rem; }
...
.cea-title { font-size: 1.45rem; font-weight: 800; color: #2d3561; margin: 0; }
.cea-title small { font-size: .75rem; font-weight: 500; color: #888; display: block; margin-top: .1rem; }
.cea-btn-new {
    display: inline-flex; align-items: center; gap: .5rem;
    background: linear-gradient(135deg, #4e73df, #224abe);
    color: #fff; border: none; border-radius: .6rem;
    padding: .6rem 1.3rem; font-weight: 700; font-size: .88rem;
    text-decoration: none; transition: opacity .2s, transform .15s;
    box-shadow: 0 4px 12px rgba(78,115,223,.35);
}
.cea-btn-new:hover { opacity: .88; transform: translateY(-1px); color: #fff; text-decoration: none; }

/* ── Empty state ── */
.cea-empty {
    text-align: center; padding: 5rem 2rem; color: #aaa;
}
.cea-empty i { font-size: 3rem; margin-bottom: 1rem; }

/* ── Course cards grid ── */
.cea-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1.5rem; }
...
.cea-card:hover { transform: translateY(-4px); box-shadow: 0 8px 28px rgba(0,0,0,.12); }
...
.cea-card-banner-icon { font-size: 3rem; color: rgba(255,255,255,.7); position: relative; z-index: 1; }
.cea-card-status {
    position: absolute; top: .7rem; left: .7rem;
    font-size: .7rem; font-weight: 700; padding: .2rem .65rem;
    border-radius: 99px; z-index: 2; backdrop-filter: blur(4px);
}
.cea-card-status.activo   { background: rgba(28,200,138,.85); color: #fff; }
.cea-card-status.inactivo { background: rgba(108,117,125,.8); color: #fff; }
.cea-card-orden {
    position: absolute; top: .7rem; right: .7rem;
    background: rgba(0,0,0,.4); backdrop-filter: blur(4px);
    color: #fff; font-size: .7rem; font-weight: 700;
    padding: .2rem .55rem; border-radius: 99px; z-index: 2;
}

/* ── Card body ── */
.cea-card-body { padding: 1.25rem 1.25rem .75rem; flex: 1; }
.cea-card-title {
    font-size: 1rem; font-weight: 800; color: #2d3561;
...
.cea-chips {
    ...
    padding: .75rem 1.25rem; border-top: 1px solid #f5f5f5;
}
.cea-chip {
...
.cea-chip.green { background: #e0fbf4; color: #1cc88a; }
.cea-chip.amber { background: #fff8e0; color: #f6c23e; }

/* ── Card actions ── */
.cea-card-actions {
    display: flex; gap: .5rem;
    padding: .75rem 1.25rem;
    border-top: 1px solid #f5f5f5; background: #fafbff;
}
.cea-act-btn {
    display: inline-flex; align-items: center; gap: .35rem;
    padding: .4rem .85rem; border-radius: .5rem;
    font-size: .78rem; font-weight: 700; text-decoration: none;
    border: none; cursor: pointer; transition: opacity .15s, transform .12s;
    flex: 1; justify-content: center;
}
.cea-act-btn:hover { opacity: .85; transform: scale(1.03); text-decoration: none; }
.cea-act-btn.view    { background: #e8f4fd; color: #36b9cc; }
.cea-act-btn.edit    { background: #fff3cd; color: #f6c23e; }
.cea-act-btn.delete  { background: #fde8e8; color: #e74a3b; }

@media (max-width: 576px) {
    .cea-grid { grid-template-columns: 1fr; }
    .cea-header { padding: 1.25rem 0 1rem; }
}
</style>
@endpush
        ...
        <div class="cea-grid" id="cea-grid">
            @foreach ($cursos as $curso)
                ...
                <div class="cea-card">
                    ...
                    <div class="cea-chips">
                        <span class="cea-chip"><i class="fas fa-layer-group"></i> {{ $totalNiveles }} nivel{{ $totalNiveles != 1 ? 'es' : '' }}</span>
                        <span class="cea-chip green"><i class="fas fa-folder-open"></i> {{ $totalUnidades }} unidad{{ $totalUnidades != 1 ? 'es' : '' }}</span>
                    </div>
...
(function () {
    ...
    gsap.set('.cea-card', { opacity: 0, y: 30 });
    gsap.to('.cea-card', {
        opacity: 1, y: 0,
        duration: .55, stagger: .1, ease: 'power3.out', delay: .1
    });
}());
</script>
@endpush
