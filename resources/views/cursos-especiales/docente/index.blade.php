...
@push('styles')
    <style>
        ...
        .ced-banner-badge.activo {
            background: rgba(28, 200, 138, .85);
            color: #fff;
        }

        .ced-banner-badge.inactivo {
            background: rgba(231, 74, 59, .82);
            color: #fff;
        }
    ...
    <div class="container-fluid ced-page">
        ...
        @if ($cursos->isNotEmpty())
            <div class="ced-chips" id="ced-chips">
                <span class="ced-chip">
                    ...
                    {{ $cursos->count() }} {{ Str::plural('curso', $cursos->count()) }}
                    asignado{{ $cursos->count() > 1 ? 's' : '' }}
                </span>
                <span class="ced-chip">
                    <i class="fas fa-layer-group"></i>
                    {{ $cursos->sum(fn($c) => $c->niveles->count()) }}
                    {{ Str::plural('niveles', $cursos->sum(fn($c) => $c->niveles->count())) }} en total
                </span>
                <span class="ced-chip">
                    <i class="fas fa-users"></i>
                    {{ $cursos->sum('inscripciones_count') }}
                    {{ Str::plural('inscripción', $cursos->sum('inscripciones_count')) }} en total
                </span>
                @if ($cursos->where('activo', true)->count())
                    <span class="ced-chip">
                        <i class="fas fa-circle" style="color:#1cc88a;font-size:.6rem"></i>
                        {{ $cursos->where('activo', true)->count() }}
                        activo{{ $cursos->where('activo', true)->count() > 1 ? 's' : '' }}
                    </span>
                @endif
            </div>
        @endif
        ...
        @if ($cursos->isEmpty())
            <div class="ced-empty">
                ...
                <p>Cuando el administrador te asigne un curso asincrónico aparecerá aquí para que puedas gestionar su
                    contenido.</p>
            </div>
        @else
                    ...
                    <div class="col-12 col-sm-6 col-lg-4 mb-4 ced-card-col">
                        <div class="ced-card">
                            ...
                            <div class="ced-banner" style="background: {{ $grad }}">
                                ...
                                @if ($curso->niveles->count())
                                    <span class="ced-banner-levels">
                                        <i class="fas fa-layer-group"></i>
                                        {{ $curso->niveles->count() }}
                                        {{ Str::plural('nivel', $curso->niveles->count()) }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="ced-body">
                                ...
                                @endif
                                <div style="font-size:.82rem;color:#6c757d;margin-top:.75rem;">
                                    <div>Inscritos: <strong>{{ $curso->inscripciones_count }}</strong></div>
                                    <div>Avance promedio: <strong>{{ $curso->promedio_avance }}%</strong></div>
                                </div>
                            </div>
